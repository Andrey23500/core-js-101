...
function doRectanglesOverlap(rect1, rect2) {
  let x1 = rect2.left;
  let y1 = rect2.top;
  let x2 = x1 + rect2.width;
  let y2 = y1 + rect2.height;
  if (rect1.left > x1) { x1 = rect1.left; }
  if (rect1.top > y1) { y1 = rect1.top; }
  if (rect1.left + rect1.width < x2) { x2 = rect1.left + rect1.width; }
  if (rect1.top + rect1.height < y2) { y2 = rect1.top + rect1.height; }
  return !((x2 <= x1 || y2 <= y1));
}
...
function isInsideCircle(circle, point) {
  ...
  let len = (point.x - circle.center.x) * (point.x - circle.center.x);
  len += (point.y - circle.center.y) * (point.y - circle.center.y);
  return len < r;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let open = '';
  let close = '';
  if (isStartIncluded) open = '[';
  else open = '(';
  if (isEndIncluded) close = ']';
  else close = ')';
...
function isBracketsBalanced(str) {
  ...
  for (let j = 0; j < str.length; j += 1) {
    if (str[j] === '(' || str[j] === '{' || str[j] === '[' || str[j] === '<') {
      stack.push(str[j]);
      i += 1;
    } else if (i >= 0 && ((stack[i] === '(' && str[j] === ')') || (stack[i] === '{' && str[j] === '}') || (stack[i] === '[' && str[j] === ']') || (stack[i] === '<' && str[j] === '>'))) {
      stack.pop();
      i -= 1;
    } else {
      return false;
...
function getCommonDirectoryPath(pathes) {
  const commonPath = pathes.reduce((acc, val) => {
    ...
    const newAcc = accSplit.reduce((ac, value, i) => {
      if (valSplit[i] !== value) {
        return ac.slice(0, i);
      }
      return ac;
